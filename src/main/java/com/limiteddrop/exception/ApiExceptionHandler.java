...
public class ApiExceptionHandler {
    ...
    ResponseEntity<ApiError> reservationBusy(ReservationBusyException e, HttpServletRequest r) {
        return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
                .header(HttpHeaders.RETRY_AFTER, "1")
                .body(apiError(HttpStatus.TOO_MANY_REQUESTS, "RESERVATION_BUSY", e.getMessage(), r, Map.of()));
    }
    ...
    ResponseEntity<ApiError> databaseUnavailable(Exception e, HttpServletRequest r) throws Exception {
        if (!DatabaseFailureClassifier.isUnavailable(e)) throw e;
