...
Container-failure testing then showed that correctness survived but availability behavior needed clearer bounds. I added short Hikari/MySQL/Redis/RabbitMQ timeouts, database-specific retryable `503` responses, MySQL-only readiness, graceful shutdown, and a two-replica HAProxy topology. Proxy retries are deliberately limited to safe HTTP methods; ambiguous mutations remain client-retried through their idempotency contract. The full suite passed with 95 tests, and live Compose checks covered MySQL, Redis, RabbitMQ, graceful replica removal, and abrupt replica failure.
